...
import { Search, Plus, Filter, X, Trash2, Volume2 as PageIcon, UploadCloud } from 'lucide-react';
...
const Speaker = () => {
  ...
  const [formData, setFormData] = useState({ title: '', room: '', time: '', });
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/data/Speaker');
  ...
  const handleSave = async () => {
    try {
      ...
      const firstField = 'title';
      if (!formData[firstField]) return alert('Announcement Title is required');
      ...
      await api.post('/data/Speaker', payload);
      setFormData({ title: '', room: '', time: '', });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Speaker</h1>
          <p className="text-slate-500 text-sm mt-1">Manage speaker records</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-slate-600 text-sm">
                ...
                <th className="p-4 font-semibold">Room / Class</th>
              ...
              {isLoading ? (
                ...
                filteredData.map((item, index) => (
                  <tr key={item._id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.room || item.title || 'N/A'}</td>
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.time || item.title || 'N/A'}</td>
                    <td className="p-4">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-800">Add New Speaker</h3>
              <button 
            ...
            <div className="p-4 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Target Room/Class</label>
                <input 
                  type="text" 
                  value={formData.room}
                  onChange={(e) => setFormData({...formData, room: e.target.value})}
                  className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" 
                  placeholder="Enter Target Room/Class..." 
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Announcement Time</label>
                <input 
                  type="time" 
                  value={formData.time}
                  onChange={(e) => setFormData({...formData, time: e.target.value})}
                  className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" 
                  placeholder="Enter Announcement Time..." 
                />
              </div>
            </div>
