...
import { Search, Plus, Filter, X, Trash2, UserCheck as PageIcon, UploadCloud } from 'lucide-react';
...
const StaffAttendance = () => {
  ...
  const [formData, setFormData] = useState({ staffName: '', date: new Date().toISOString().split('T')[0], status: '', });
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/data/StaffAttendance');
  ...
  const handleSave = async () => {
    try {
      ...
      const firstField = 'staffName';
      if (!formData[firstField]) return alert('Staff Name is required');
      ...
      await api.post('/data/StaffAttendance', payload);
      setFormData({ staffName: '', date: new Date().toISOString().split('T')[0], status: '', });
              ...
              {isLoading ? (
                ...
                filteredData.map((item, index) => (
                  <tr key={item._id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.staffName || item.title || 'N/A'}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-800">Add New Staff Attendance</h3>
              <button 
