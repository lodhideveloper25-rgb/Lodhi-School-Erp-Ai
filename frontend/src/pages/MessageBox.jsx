...
import { Search, Plus, Filter, X, Trash2, Mail as PageIcon, UploadCloud } from 'lucide-react';
...
const MessageBox = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/data/MessageBox');
  ...
  const handleSave = async () => {
    try {
      ...
      const firstField = 'recipient';
      if (!formData[firstField]) return alert('Recipient is required');
      ...
      await api.post('/data/MessageBox', payload);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Message Box</h1>
          <p className="text-slate-500 text-sm mt-1">Manage message box records</p>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-800">Add New Message Box</h3>
              <button 
