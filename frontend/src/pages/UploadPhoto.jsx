...
const UploadPhoto = () => {
  ...
  const [formData, setFormData] = useState({ albumName: '', photoFile: '', });
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/data/UploadPhoto');
  ...
  const handleSave = async () => {
    try {
      ...
      const firstField = 'albumName';
      if (!formData[firstField]) return alert('Album / Caption is required');
      ...
      await api.post('/data/UploadPhoto', payload);
      setFormData({ albumName: '', photoFile: '', });
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage upload photo records</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-slate-600 text-sm">
                ...
                <th className="p-4 font-semibold">Album / Caption</th>
                <th className="p-4 font-semibold">Photo Name</th>
              ...
              {isLoading ? (
                ...
                filteredData.map((item, index) => (
                  <tr key={item._id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.albumName || item.title || 'N/A'}</td>
                    <td className="p-4 text-sm text-blue-500 hover:underline cursor-pointer">{item.metadata?.photoFile || 'File Uploaded'}</td>
                    <td className="p-4">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-800">Add New Upload Photo</h3>
              <button 
            ...
            <div className="p-4 space-y-4 overflow-y-auto">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Album / Caption</label>
                <input 
                  type="text" 
              ...
              <div>
                ...
                <div 
                  onClick={() => document.getElementById('file_photoFile').click()}
                ...
                >
                  ...
                  <span className="text-sm font-medium text-slate-700">
                    {formData.photoFile ? formData.photoFile : 'Click to upload file'}
                  </span>
