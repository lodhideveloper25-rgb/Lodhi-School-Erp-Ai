...
const Staff = () => {
  ...
  const [formData, setFormData] = useState({ staffId: '', name: '', role: '', department: '', });
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/data/Staff');
  ...
  const handleSave = async () => {
    try {
      ...
      const firstField = 'staffId';
      if (!formData[firstField]) return alert('Staff ID is required');
      ...
      setFormData({ staffId: '', name: '', role: '', department: '', });
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage staff records</p>
        </div>
              ...
              {isLoading ? (
                ...
                filteredData.map((item, index) => (
                  <tr key={item._id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.staffId || item.title || 'N/A'}</td>
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.name || item.title || 'N/A'}</td>
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.role || item.title || 'N/A'}</td>
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.department || item.title || 'N/A'}</td>
                    <td className="p-4">
