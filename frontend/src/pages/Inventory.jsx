...
import { Search, Plus, Filter, X, Trash2, Box as PageIcon, UploadCloud } from 'lucide-react';
...
const Inventory = () => {
  ...
  const [formData, setFormData] = useState({ itemName: '', category: '', quantity: '', unitPrice: '', });
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/data/Inventory');
  ...
  const handleSave = async () => {
    try {
      ...
      const firstField = 'itemName';
      if (!formData[firstField]) return alert('Item Name is required');
      ...
      await api.post('/data/Inventory', payload);
      setFormData({ itemName: '', category: '', quantity: '', unitPrice: '', });
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage inventory records</p>
        </div>
              ...
              {isLoading ? (
                ...
                filteredData.map((item, index) => (
                  <tr key={item._id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.itemName || item.title || 'N/A'}</td>
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.category || item.title || 'N/A'}</td>
                    <td className="p-4 text-sm font-semibold text-slate-700">{item.metadata?.quantity || '0'}</td>
                    <td className="p-4 text-sm font-semibold text-slate-700">{item.metadata?.unitPrice || '0'}</td>
                    <td className="p-4">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-800">Add New Inventory</h3>
              <button 
