...
const StudentIdCard = () => {
  ...
  const [formData, setFormData] = useState({ studentId: '', templateType: '', issueDate: new Date().toISOString().split('T')[0], });
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await api.get('/data/StudentIdCard');
  ...
  const handleSave = async () => {
    try {
      ...
      await api.post('/data/StudentIdCard', payload);
      setFormData({ studentId: '', templateType: '', issueDate: new Date().toISOString().split('T')[0], });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Student Id Card</h1>
          <p className="text-slate-500 text-sm mt-1">Manage student id card records</p>
        </div>
              ...
              {isLoading ? (
                ...
                filteredData.map((item, index) => (
                  <tr key={item._id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.templateType || item.title || 'N/A'}</td>
                    <td className="p-4 text-sm text-slate-700">{item.metadata?.issueDate ? new Date(item.metadata.issueDate).toLocaleDateString() : 'N/A'}</td>
                    <td className="p-4">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <h3 className="font-bold text-lg text-slate-800">Add New Student Id Card</h3>
              <button 
            ...
            <div className="p-4 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Template Design</label>
                <select 
                  value={formData.templateType}
                  onChange={(e) => setFormData({...formData, templateType: e.target.value})}
                  className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                >
                  <option value="">Select Template Design</option>
